import './style.css'
    import { startKakaoLogin, startGoogleLogin, login, logout, signUp } from '.
  /auth/auth-api.js'
    ...
    import { renderPersonalitySurveyPage } from './pages/personality-survey.js'
  ...
  'http://localhost:8080').replace(/\/$/, '')
    ...
    export function showToast(message) {
      ...
      toast.className = 'fixed bottom-6 left-1/2 -translate-x-1/2 bg-brand-navy 
  text-white px-6 py-3.5 rounded-full text-xs sm:text-sm font-bold shadow-floating 
  z-50 animate-bounce flex items-center gap-2 border border-white/10'
      toast.innerHTML = `
        <span class="material-symbols-outlined text-primary-container text-base
  sm:text-lg">warning</span>
        <span>${message}</span>
      `
    ...
    const routeApp = async () => {
      ...
      if (path === '/oauth/callback') {
        ...
        renderPersonalitySurveyPage(app)
      } else {
        // 기본 메인 랜딩 페이지
        initLandingPage()
      }
    ...
    function initL    const token = getAccessToken()
    function initLandingPage() {
      ...
      const showLoginForm = () => {
        ...
        if (modalDesc) modalDesc.textContent = '혼밥 말고 따뜻한 한 끼를 함께할
  친구를 만나보세요.'
        if (loginErrorMsg) loginErrorMsg.style.display = 'none'
      }
      ...
      const showSignupForm = () => {
        ...
        if (modalDesc) modalDesc.textContent = '간단한 가입으로 나만의 1:1 밥친구를
  찾아보세요.'
        if (signupErrorMsg) signupErrorMsg.style.display = 'none'
      }
      ...
      if (token) {
        headerAuth.innerHTML = `
          <div class="flex items-center gap-2 sm:gap-3">
            <a href="/personality/survey" class="hidden sm:inline-flex items-center
  gap-1.5 px-3 py-1.5 rounded-full bg-slate-100 hover:bg-slate-200 text-brand-navy
  text-xs font-bold transition-colors">
              <span class="material-symbols-outlined text-sm text-primary-
  container">psychology</span>
              <span>식사 성향</span>
            </a>
            <span class="hidden sm:inline-flex items-center gap-1.5 px-3 py-1
  rounded-full bg-success/15 text-success text-xs font-bold">
              <span class="w-1.5 h-1.5 rounded-full bg-success"></span>
              로그인 됨
            </span>
            <button id="btn-revoke-location" class="btn-secondary px-3 sm:px-4 py-1.
  5 sm:py-2 rounded-full text-xs sm:text-sm font-semibold text-error hover:bg-
  error/10 hover:text-error border-error/30 flex items-center gap-1">
              <span class="material-symbols-outlined text-sm">no_accounts</span>
              <span>위치동의 철회</span>
            </button>
            <button id="btn-logout" class="btn-secondary px-3 sm:px-4 py-1.5 sm:py-2
  rounded-full text-xs sm:text-sm font-semibold">
        ...
        // 위치 동의 철회 버튼 바인딩
        document.querySelector('#btn-revoke-location')?.addEventListener('click',
  async (e) => {
          e.preventDefault()
          e.stopPropagation()
          if (confirm('위치 정보 이용 동의를 철회하시겠습니까?\n철회 시 등록된
  선호위치와 대기 중인 모든 매칭 요청이 파기됩니다.')) {
            try {
              const csrfResp = await fetch(`${API_BASE_URL}/auth/csrf`, {
  credentials: 'include' })
  ...
  => {
        ...
        headerAuth.innerHTML = `
          <div class="flex items-center gap-3">
            <button id="btn-header-login" class="btn-secondary px-4 py-2 rounded-
  full text-xs sm:text-sm font-semibold">
              로그인
            </button>
            <button id="btn-header-start" class="btn-primary px-4 py-2 rounded-full
  text-xs sm:text-sm font-semibold flex items-center gap-1.5 shadow-md">
              <span>시작하기</span>
        ...
        const openAuthModal = (isSignup = false) => {
          ...
          if (isSignup) {
            ...
            if (modalTitle) modalTitle.textContent = '이메일 회원가입'
            if (modalDesc) modalDesc.textContent = '간단한 가입으로 나만의 1:1
  밥친구를 찾아보세요.'
          } else {
  ...
  openAuthModal(false))
        document.querySelector('#btn-header-start')?.addEventListener('click', () =>
  openAuthModal(false))
      }
    }
      ...
      container.innerHTML = `
        <main class="page-shell">
          <section class="auth-card" aria-labelledby="profile-title">
            ...
            <p class="description">
              임시 닉네임이 발급되었습니다. 나만의 식사 성향과 취향을 설정하여
  완벽한 마주한끼를 만나보세요.
            </p>
