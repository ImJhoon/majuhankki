...
- 인증 계정과 자격 증명은 **Spring Security 기반 인증 모듈**이 직접 관리한다. `users`에는 해시된 비밀번호만 저장하고, 카카오 계정 연결은 `oauth_accounts`, Refresh Token은 원문 대신 해시를 `refresh_tokens`에 저장한다.
...
| 소셜 로그인 계정 | `oauth_accounts` | 카카오 등 외부 OAuth2 계정과 회원의 연결 정보 |
| Refresh Token | `refresh_tokens` | JWT 재발급을 위한 회전·폐기 가능한 토큰 정보 |
...
erDiagram
    ...
    USERS ||--o{ OAUTH_ACCOUNTS : "소셜계정 연결"
...
| 속성 | 논리 타입    | 키/제약 | 설명 |
| --- |----------| --- | --- |
| `id` | UUID | PK | 서버가 생성하는 회원 식별자 |
| `email` | VARCHAR(255) | UNIQUE, NULL 허용 | 이메일 로그인 식별자. 카카오에서 이메일을 제공하지 않는 계정은 NULL 허용 |
| `password_hash` | VARCHAR(255) | NULL 허용 | `PasswordEncoder`로 해싱한 비밀번호. 소셜 로그인 전용 계정은 NULL |
...
### `oauth_accounts` — FR-01-02

| 속성 | 논리 타입 | 키/제약 | 설명 |
| --- | --- | --- | --- |
| `id` | INTEGER | PK | 소셜 계정 연결 식별자 |
| `user_id` | UUID | FK → users.id | 서비스 회원 |
| `provider` | VARCHAR(30) | NOT NULL | `KAKAO` 등 OAuth2 제공자 |
| `provider_user_id` | VARCHAR(255) | NOT NULL | 제공자가 발급한 사용자 식별자 |
| `created_at` | DATETIME | NOT NULL | 연결 시각 |

`(provider, provider_user_id)` 조합은 유일해야 하며 OAuth Access/Refresh Token 원문은 저장하지 않는다.

### `refresh_tokens` — FR-01-02

| 속성 | 논리 타입 | 키/제약 | 설명 |
| --- | --- | --- | --- |
| `id` | UUID | PK | 토큰 레코드 식별자 |
| `user_id` | UUID | FK → users.id | 토큰 소유 회원 |
| `token_hash` | VARCHAR(255) | UNIQUE, NOT NULL | 원문 Refresh Token의 단방향 해시 |
| `expires_at` | DATETIME | NOT NULL | 만료 시각 |
| `revoked_at` | DATETIME | NULL 허용 | 로그아웃·재발급·탈퇴에 따른 폐기 시각 |
| `created_at` | DATETIME | NOT NULL | 발급 시각 |

재발급할 때 기존 Refresh Token을 폐기하고 새 토큰을 발급하는 회전 방식을 사용한다.
...
- 인증 데이터는 Spring Security 기반 인증 모듈이 관리하며, 비밀번호는 `PasswordEncoder` 해시만 저장한다. 카카오 계정과 Refresh Token은 각각 별도 테이블로 분리한다.
...
| UUID | `uuid` | 회원·Refresh Token 식별자 및 모든 사용자 참조 키 |
...
| 테이블 | 물리 설계 추가사항 |
| --- | --- |
| `users` | UUID PK, 이메일·비밀번호 해시·프로필·권한·상태 관리; `location geography(Point,4326)` 및 GIST 인덱스 추가 |
| `oauth_accounts` | `(provider, provider_user_id)` UNIQUE 제약으로 동일 카카오 계정의 중복 연결 차단 |
| `refresh_tokens` | `token_hash` UNIQUE 인덱스와 `(user_id, revoked_at, expires_at)` 인덱스로 검증·전체 로그아웃 처리 가속화 |
