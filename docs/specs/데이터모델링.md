...
| 15 | 사용자 성향 임베딩 | `user_personality_embeddings` | 동의한 자유 서술 성향의 보조 랭킹 벡터 |
...
- **PERSONALITY**:
  ...
  - `user_personality_profiles` (1) : (0..1) `user_personality_embeddings`
- **MATCH_REQUEST**:
...
erDiagram
    ...
    user_personality_profiles ||--o| user_personality_embeddings : "has_embedding"
    ...
    matches {
        bigint id PK
        bigint request_1_id FK, UK
        bigint request_2_id FK, UK
        varchar status
...
| `self_description` | VARCHAR(300) | NULL | AI 분석에 동의한 사용자의 선택형 자기 스타일 설명 |
...
| 컬럼명 | 논리 타입 | 제약 조건 | 설명 |
| :--- | :--- | :--- | :--- |
| `user_id` | UUID | PK, FK -> `user_personality_profiles.user_id` | 임베딩 소유 사용자 |
| `source_text` | TEXT | NOT NULL | 동의한 자기소개 자유 텍스트만 저장한 버전 문서. 카드 점수·확정 태그는 포함하지 않으며 상대방에게 노출하지 않음 |
