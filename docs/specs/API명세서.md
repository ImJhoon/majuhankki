...
비로그인 체험용 데모는 고정된 로컬 시나리오만 사용하며 백엔드 API를 추가하거나 호출하지 않는다. 운영 인증·매칭·채팅 API와 WebSocket 및 쿠키 정책은 변경하지 않는다.
...
- `password`: 필수, 8자 이상 100자 이하, 영문·숫자·특수기호를 각각 1자 이상 포함하며 공백은 사용할 수 없음
...
성향 프로필 응답은 본인에게 온보딩 상태, 계산 점수, 확정 태그, 저장된 자기소개·AI 키워드와 현재 동의 상태를 제공한다. `aiKeywords`는 자유소개에서 AI가 추출한 최대 5개의 키워드이며 `styleTags`와 별도로 저장·표시한다. 원본 카드 응답, 임베딩 및 차원별 내부 랭킹 자료는 이 API나 상대방용 응답에 포함하지 않는다. 자기소개, AI 요청·응답, 임베딩 문서는 애플리케이션 로그에 기록하지 않는다. 점수는 식사 선호의 방향만 나타내고 높고 낮음을 좋은·나쁜 성향으로 해석하지 않는다.

AI 분석에 동의한 프로필 제출은 정규화된 `aiKeywords`가 최소 1개 있어야 한다. `aiAnalysisConsent=false`인 제출은 키워드가 없어도 허용하며, 전달된 자기소개와 AI 키워드는 폐기하고 기존 파생 임베딩도 삭제한다. AI 동의 철회 후에도 온보딩을 완료하고 확정 태그가 있는 사용자는 매칭을 요청할 수 있다.
...
`compatibility`는 저장된 호환도 스냅샷이 있을 때만 포함하며, `myScore`는 수신자의 희망 조건에서 상대방을 본 방향의 점수, `partnerScore`는 상대방의 희망 조건에서 수신자를 본 방향의 점수다. `score`는 정렬과 기존 호환성을 위한 양방향 최종 점수이며, 성향 데이터가 없는 방향은 기본 조건 fallback 사유로 표시할 수 있다. `desiredLocations.mine`과 `desiredLocations.partner`는 상호 수락이 끝난 매칭 참여자에게만 수신자 관점으로 제공하며, `match_requests.location`에 저장된 요청 시점의 희망 핀과 `foodCategory`이지 사용자의 실시간 위치가 아니다. 후보 제안 단계에는 이 좌표를 노출하지 않는다. 프런트엔드는 두 핀의 중간 지점 반경 1.5km에서 각 음식 유형별 키워드로 Kakao 장소를 검색하되 음식점은 `FD6`, 카페는 `CE7` 그룹으로 제한하고 키워드별 최대 3페이지·45곳을 정확도순 조회한다. 응답의 `category_name`을 선택 유형 분류어와 다시 대조하고 도로명 주소·전화번호·상세 URL 중 2개 이상이 있는 장소만 남긴다. 서로 다른 유형은 어느 한쪽 후보가 5곳 미만이면, 같은 유형은 후보가 10곳 미만이면 반경 3km로 다시 조회한다. 실제 적용 반경을 기준으로 카테고리 정확성 40%, 중간 지점 거리 30%, 양쪽 거리 균형 20%, 정보 완성도 10%로 유형별 점수화하고, 서로 다른 유형은 사용자별 최대 5곳씩, 같은 유형은 통합 최대 10곳을 표시한다. 양쪽 유형에 함께 해당하는 동일 장소 ID는 소유자를 합쳐 마커 하나로 표시한다. 상호명에 `한식`, `중식` 등의 문자열이 포함된 것만으로는 통과시키지 않는다. 이 결과는 평점·리뷰 품질을 보장하지 않는 주변 후보이며 영업 여부·품질·예약 가능성을 보장하지 않는다. 원본 설문 답변, 자유 서술, 차원별 상세 점수와 임베딩 벡터는 상대방에게 노출하지 않는다. 동일한 결과를 재전송하더라도 매칭·채팅방 레코드를 중복 생성하지 않는다.
...
매칭 요청 생성 트랜잭션에서는 AI 키워드 추출·임베딩을 동기 호출하지 않는다. 요청 저장 후 커밋 이벤트를 통해 희망 텍스트 임베딩을 비동기로 생성하며, 동기·비동기 경로에서 같은 텍스트를 중복 생성하지 않는다. 하드 필터를 통과한 후보는 프로필·태그·AI 키워드가 없다는 이유만으로 제외하지 않고 계산 가능한 신호 또는 기본 조건 점수를 사용한다.
...
`GET /matches/history` 응답의 `reported`는 현재 사용자가 해당 매칭의 상대방을 이미 신고했는지를 나타낸다. `true`인 경우 클라이언트는 신고 버튼을 `신고 접수 완료` 상태로 표시하고 재신고를 허용하지 않는다. 관리자 기각·처리 후에도 신고 이력은 보존된다.
