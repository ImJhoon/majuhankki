...
- [ ] 핀이 선택 구 범위를 벗어날 경우 에러 코드 `LOCATION_002` 반환
- [ ] 기준 핀 좌표로부터 반경 N km 이내의 매칭 대기글을 조회하는 API 구현 (`GET /api/matches/nearby`)
...
- [ ] 선호 지역 조회 API 결과로 지도 중심 초기화 및 필터 연동 테스트 수행
- [ ] 입력받은 위경도로 JTS Point 객체가 올바른 순서와 SRID로 빌드되는지 단위 테스트 수행
- [ ] 지정한 시군구 경계 내에 위치 좌표가 포함되는지 여부를 판단하는 경계 검증 테스트 수행
- [ ] PostGIS `ST_DWithin`을 활용한 반경 N km 이내 대상 필터링 쿼리 단위 테스트 수행
- [ ] PostGIS `ST_Distance`를 활용한 내 핀과의 거리 정렬 및 미터 단위 거리 계산 테스트 수행
- [ ] 프론트엔드에서 핀 드래그 시 좌표 값이 상태값으로 정상 반환되는지 확인
